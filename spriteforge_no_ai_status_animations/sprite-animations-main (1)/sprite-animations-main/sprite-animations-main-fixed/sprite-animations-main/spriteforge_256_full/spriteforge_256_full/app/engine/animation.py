...
class SpriteAnimator:
    ...
    def _sprite_tint(self, frame: Image.Image, color=(255, 48, 36, 72)) -> Image.Image:
        """Apply a transparent color flash only over visible sprite pixels."""
        overlay = Image.new('RGBA', frame.size, color)
        alpha = frame.getchannel('A').point(lambda value: int(value * (color[3] / 255)))
        overlay.putalpha(alpha)
        return Image.alpha_composite(frame, overlay)

    def _soft_glow(self, frame: Image.Image, color=(255, 255, 255, 90), radius: int = 8, strength: float = 1.0) -> Image.Image:
        """Reusable outline glow derived from the sprite alpha."""
        alpha = frame.getchannel('A').filter(ImageFilter.GaussianBlur(radius))
        glow = Image.new('RGBA', frame.size, color)
        glow.putalpha(alpha.point(lambda value: int(value * strength * (color[3] / 255))))
        out = Image.new('RGBA', frame.size, (0, 0, 0, 0))
        out.alpha_composite(glow)
        out.alpha_composite(frame)
        return out

    def _impact_body(self, t: float, strength: float = 1.0, direction: int = -1) -> Image.Image:
        kick = max(0.0, 1.0 - t)
        snap = math.sin(kick * math.pi)
        return affine(
            self.base,
            1.0 - 0.012 * kick * strength,
            1.0 + 0.01 * kick * strength,
            shear=0.012 * kick * direction,
            angle=self._amp(direction * (1.5 if t < 0.5 else -0.7) * kick * strength),
            translate=(self._move_x(self._amp(direction * 14 * kick * strength)), self._amp(-2.0 * snap * strength)),
        )
        ...
        for i in range(self.s.frames):
            ...
            if t < 0.34:
                ...
                body = affine(
                    ...
                    shear=-0.015 * draw,
                    angle=-4.0 * draw,
                    translate=(self._move_x(self._amp(-4.0 * draw)), self._amp(1.0 * draw)),
                )
            elif t < 0.68:
                hold = (t - 0.34) / 0.34
                body = affine(
                    ...
                    shear=-0.01,
                    angle=-2.0,
                    translate=(self._move_x(self._amp(-5.5)), self._amp(0.5)),
                )
            else:
                release = (t - 0.68) / 0.32
                body = affine(
                    ...
                    shear=0.02 * release,
                    angle=5.0 * release,
                    translate=(self._move_x(self._amp(7.0 * release)), self._amp(-1.5 * release)),
                )
            frames.append(body)
        return frames
    ...
    def _anim_damage(self):
        ...
        for i in range(self.s.frames):
            t = self._t(i)
            kick = max(0.0, 1.0 - t)
            pulse = 1.0 if i < max(2, self.s.frames // 3) else max(0.0, 1.0 - t * 1.35)
            body = self._impact_body(t, strength=0.85, direction=-1)
            if pulse > 0.05:
                body = self._sprite_tint(body, (255, 36, 28, int(82 * pulse)))
            frames.append(body)
        return frames
    ...
    def _anim_heal(self):
        ...
        for i in range(self.s.frames):
            ...
            body = affine(
                ...
                1.0 + 0.012 * pulse,
                1.0 - 0.012 * pulse,
                translate=(self._move_x(0.6 * math.sin(p * 0.5)), self._amp(-2.2 * pulse)),
            )
            body = self._soft_glow(body, (185, 255, 205, 95), radius=9, strength=0.75 * pulse)
            body = self._sprite_tint(body, (255, 255, 255, int(92 * pulse)))
            frames.append(body)
        return frames
    ...
    def _anim_buff(self):
        ...
        for i in range(self.s.frames):
            p = self._phase(i)
            pulse = 0.35 + 0.65 * abs(math.sin(p))
            body = affine(
                self.base,
                1.0 + 0.01 * pulse,
                1.0 - 0.008 * pulse,
                angle=0.35 * math.sin(p * 0.5),
                translate=(0, self._amp(-1.4 * pulse)),
            )
            body = self._soft_glow(body, (255, 225, 120, 82), radius=7, strength=0.65 * pulse)
            body = self._sprite_tint(body, (255, 238, 160, int(45 * pulse)))
            frames.append(body)
        return frames
    ...
    def _anim_shield(self):
        ...
        for i in range(self.s.frames):
            p = self._phase(i)
            pulse = 0.35 + 0.65 * abs(math.sin(p))
            body = affine(self.base, 1.0 + 0.004 * math.sin(p), 1.0 - 0.004 * math.sin(p), translate=(0, self._amp(0.8 * math.sin(p))))
            aura = self._aura(self._t(i), center=(128, 130), radius=48, color=(115, 190, 255, int(70 * pulse)))
            body = self._soft_glow(body, (120, 205, 255, 70), radius=6, strength=0.45 * pulse)
            frames.append(self._compose(body, [aura]))
        return frames
    ...
    def _anim_poison(self):
        ...
        for i in range(self.s.frames):
            p = self._phase(i)
            wobble = math.sin(p)
            pulse = 0.45 + 0.55 * abs(math.sin(p * 0.5))
            body = affine(
                self.base,
                1.0 - 0.006 * pulse,
                1.0 + 0.006 * pulse,
                shear=0.01 * wobble,
                angle=0.9 * wobble,
                translate=(self._move_x(1.2 * wobble), self._amp(1.0 * math.cos(p))),
            )
            body = self._sprite_tint(body, (105, 230, 90, int(58 * pulse)))
            body = self._soft_glow(body, (120, 255, 90, 55), radius=5, strength=0.35 * pulse)
            frames.append(body)
        return frames
