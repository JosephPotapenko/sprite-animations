...
def _backend_mode() -> str:
    ...
    The free/default backend must always work in Codespaces and on local CPU.
    Diffusion is intentionally opt-in because it needs large model downloads and
    usually a GPU. Set SPRITEFORGE_AI_BACKEND=diffusion to try it.
    """
    backend = os.getenv('SPRITEFORGE_AI_BACKEND', 'procedural').strip().lower()
    if backend in {'diffusion', 'procedural'}:
        return backend
    return 'procedural'
...
def _procedural_ai_frames(base: Image.Image, prompt: str, frames: int, size: int, animation: str, seed: int | None) -> list[Image.Image]:
    ...
    for frame_index in range(frame_total):
        motion_source = _motion_source(source, animation, frame_index, frame_total)
...
def _run_diffusion_frames(base: Image.Image, prompt: str, frames: int, size: int, animation: str, seed: int | None) -> list[Image.Image]:
    ...
    for frame_index in range(max(2, min(32, frames))):
        motion_source = _motion_source(source, animation, frame_index, frames)
...
def generate_ai_frames(base: Image.Image, prompt: str, frames: int = 8, size: int = 256, animation: str = 'custom', seed: int | None = None) -> list[Image.Image]:
    ...
    if backend == 'diffusion' and is_available():
        try:
            return _run_diffusion_frames(base, prompt, frames, size, animation, seed)
        except Exception:
            # Never break the web app just because the experimental diffusion
            # layer failed. The procedural prompt fallback is deterministic and
            # works on free CPU machines.
