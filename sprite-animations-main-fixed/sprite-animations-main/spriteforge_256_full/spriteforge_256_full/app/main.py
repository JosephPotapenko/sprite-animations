...
# The AI endpoint is safe to expose by default because the default backend is a
# CPU-only procedural prompt engine. Real diffusion is still opt-in with
# SPRITEFORGE_AI_BACKEND=diffusion and optional requirements-ai.txt.
AI_FEATURE_FLAG = os.getenv('SPRITEFORGE_ENABLE_AI', '1').strip().lower() not in {'0', 'false', 'no', 'off'}
...
def ai_status():
    ...
    return {
        ...
        'message': (
            'AI generation has been disabled with SPRITEFORGE_ENABLE_AI=0.'
            if not AI_FEATURE_FLAG
            else (
                'Prompt-guided CPU AI fallback is available.'
                if default_model_id() == 'procedural-prompt-fallback'
                else ('Diffusion backend is selected but dependencies/model may be unavailable.' if not available else 'Optional diffusion generation is available.')
            )
        ),
    }
...
async def generate(
    ...
    try:
        req = GenerateRequest(
            animation=animation if animation in ['idle', 'walk', 'run', 'jump', 'attack', 'ranged', 'cast', 'hurt', 'spin', 'bounce', 'custom'] else 'custom',
            prompt=prompt,
            frames=frames,
            fps=fps,
...
async def generate_ai(
    ...
    try:
        parsed = parse_prompt(prompt, animation, frames, fps)
        frame_total = max(2, min(32, int(parsed.frames)))
        fps_value = max(1, min(30, int(parsed.fps)))
        seed_value = int(seed) if str(seed).strip() else None
        frames_out = _normalize_frames(
            generate_ai_frames(
                base,
                prompt,
                frames=frame_total,
    ...
    save_gif(frames_out, gif_path, fps_value)
